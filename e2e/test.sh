...
BASE="${AIRYFS_URL:-https://airyfs.tyson-s-sandbox.workers.dev}"
VOL="e2e-$(date +%s)"
PASS=0
FAIL=0
...
GIT_RESULT=$(curl -sf --max-time 30 -X POST "$BASE/exec?volume=$VOL" \
  ...
  -d '{"command":"cd /volume && git init && echo hello > main.py && git add -A && git -c user.name=airyfs -c user.email=airyfs@test commit -m init 2>&1"}')
