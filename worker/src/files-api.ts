...
export interface V1Route {
  volume: string;
  resource: 'volume' | 'forks' | 'sql' | 'files' | 'directories' | 'trees' | 'tree' | 'operations' | 'exec' | 'usage' | 'quota' | 'trash' | 'services' | 'capabilities' | 'snapshots' | 'uploads' | 'browser-uploads' | 'assets' | 'jobs' | 'schedules' | 'search' | 'changes' | 'webhooks' | 'auth' | 'sites' | 'shares';
  path: string;
}
...
export function parseV1Route(pathname: string): V1Route | null {
  ...
  if (!['forks', 'sql', 'files', 'directories', 'trees', 'tree', 'operations', 'exec', 'usage', 'quota', 'trash', 'services', 'capabilities', 'snapshots', 'uploads', 'browser-uploads', 'assets', 'jobs', 'schedules', 'search', 'changes', 'webhooks', 'auth', 'sites', 'shares'].includes(resource)) {
