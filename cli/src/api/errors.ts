// ABOUTME: Normalizes JSON and plain-text errors returned by the AiryFS Worker.
// ABOUTME: Preserves HTTP status and server error code for command-level handling.
...
export async function responseError(response: Response): Promise<AiryFSApiError> {
  ...
  if (body) {
    try {
      ...
      // The Worker also has a few legacy plain-text error responses.
    }
  }
