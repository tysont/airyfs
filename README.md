...
Until then a mounted subtree inside the Container is a **read-only** stub directory (a kernel bind remount — no daemon — so it never touches the failing path) carrying an `AIRYFS-MOUNT-UNAVAILABLE.txt` marker: reads surface the marker and writes fail with `EROFS` rather than silently landing in a soon-to-be-shadowed local stub. Direct-path reads and writes are fully live and unaffected.
