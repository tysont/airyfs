...
A single volume is bounded by one Durable Object's SQLite storage. To grow past that limit, mount another volume at a subdirectory: `PUT /v1/volumes/A/mounts/data` with `{"target":"big-2","create":true}` grafts volume `big-2` at `/data` inside volume `A`, creating `big-2` in the same request. Reads and writes under `/data` are served by `big-2`'s Durable Object; everything else stays in `A`. Each mounted volume keeps its own independent quota, snapshots, and parallelism, so `N` volumes give roughly `N` volumes' worth of addressable space under one namespace. Without `create`, the target must already exist — mounting a non-existent (e.g. typo'd) target is refused with `MOUNT_TARGET_NOT_FOUND` rather than silently mounting an auto-instantiated empty volume that would swallow writes.

Routing happens on the path plane, never by rewriting SQL. Direct-path requests (HTTP, Workers RPC, SDK, CLI, S3, WebDAV) resolve the longest matching mountpoint in the host volume's mount table and forward the operation to the target volume's Durable Object, translating the path and carrying a scoped, revocable capability minted on the target. AiryFS itself takes zero patches; the router it relies on (the wrapper layer) already exists. This direct-path plane is the primary interface and covers reads, writes, ranges, listings, checksums, appends, and streams under a mount. Forwarding costs one extra Durable Object hop — on the order of tens of milliseconds per operation (~60ms observed on the integration environment for a small read) — which the streaming paths amortize over the transfer. If a target is deleted or transiently fails, a forwarded operation returns the structured `MOUNT_TARGET_UNAVAILABLE` (503) rather than a raw 5xx or a false-empty listing, and mount health surfaces per mount in `/usage` and as the `airyfs_mount_healthy` gauge in `/metrics` so a degraded mount is discoverable.

In-container visibility of mounted subtrees (so `exec` sees another volume's data through FUSE) is designed as one `agentfs` FUSE daemon per volume grafted over the mountpoint's stub directory, with the guest volume's Hrana traffic proxied through the host Durable Object to the target's SQLite so it never leaves the deployment. That guest data path is implemented and unit-tested but currently gated off (`GUEST_FUSE_ENABLED`). Discriminator experiments on a deployed environment localized the failure to running a second `agentfs` FUSE daemon in the Container at all: it wedges the whole command-server process (which also hosts the bridge, so both FUSE data planes stall) and the DO recycles it. Ruled out by observation, not assumption: a platform port/listener cap (extra in-process channels leave a plain exec stable), the host DO opening extra connections (idle probes are stable), the DO-side forwarder wiring (with forwarders active but the guest daemon not spawned, a mounted exec succeeds), FUSE nesting (a guest daemon at a non-nested path fails identically), the cross-DO RPC hop (a null forwarder answering from the host's own SQLite still hangs — so connecting the guest bridge directly to the target would inherit the bug), the host DO itself (its metrics stay responsive throughout the hang), and the Container tier (`standard-2` hangs like `standard-1`). It is a hang, not a crash. Neither bridge-port multiplexing nor a direct-to-target transport addresses it; re-enabling requires resolving the two-daemon interaction at the `agentfs`/FUSE level.

In-container visibility is enabled: `exec` (and jobs and scheduled runs) read and write a mounted volume's data through the nested FUSE mount, with writes landing in the target — verified end to end on the integration environment. Each guest-FUSE operation crosses container → host Durable Object → target Durable Object, so it carries a per-operation hop on the order of the ~60ms direct-path forward above; the streaming paths amortize it. Exec admission waits for every guest mount to become live before a command runs, so a command never sees a half-mounted subtree; if a mount cannot come up within its bound the command fails with `MOUNT_TARGET_UNAVAILABLE` (retryable, `Retry-After`) rather than running against an empty stub, and the Container is left running so the retry finds the mount ready without a cold restart. The command server enforces an invariant — no synchronous filesystem call on a FUSE path from its event loop — because that was the single bug behind the original container wedge; a separate-process `/proc` watchdog (permanent) is what localized it. Should the feature be disabled (`GUEST_FUSE_ENABLED`), a mounted subtree inside the Container degrades to a **read-only** stub directory carrying an `AIRYFS-MOUNT-UNAVAILABLE.txt` marker (reads surface the marker, writes fail `EROFS`) rather than silently shadowing data. Direct-path reads and writes are unaffected either way.

Boundaries follow POSIX. Operations wholly inside one volume — reads, writes, ranges, checksums, appends, streams — forward transparently. Cross-boundary `rename` and hard `link` return `EXDEV`, exactly as across Linux filesystems, so a client `mv` falls back to copy-and-delete. A delete under a mountpoint forwards to the target before any local trash move, so it lands in the target volume's trash and is undone there. Snapshots, forks, trash, quotas, and site publishing are per-volume by design: a host volume's snapshot captures its own filesystem, never the target's data. The mount table (`fs_mount`) lives outside the snapshotted filesystem, so a restore or site rollback preserves the mounts and re-establishes any stub directory the restored snapshot predates. Fork and snapshot-clone deliberately do **not** carry the mount table: they copy the source filesystem into a different volume, and a mount's stored credential is scoped to and held by the source, so replaying it into a fork would silently create a second writer against the same target under the source's credential. A fork or clone therefore materializes former mountpoints as plain empty directories, independent and writable in the new volume; re-mount explicitly if the copy should share the target. Forwarded operations carry a bounded hop counter (and mount creation walks the target's mount graph) so `A→B→A` loops and deep `A→B→C` chains are rejected. Search, glob, tree, and change-feed run only against the host volume's own filesystem, so when their scope contains a mountpoint they do not descend into the target's data. Rather than return a silently incomplete result, they set an `X-AiryFS-Truncated-At-Mounts` response header listing the omitted mountpoints. `du` reports the host subtree's own usage (a mountpoint under the `du` path counts as its stub); a `du` whose path falls under a mount forwards to the target and reports that subtree in full. One known limitation: a symbolic link in the host volume whose target points into a mounted subtree resolves correctly through FUSE (the kernel walks it) but not through the direct path, which does not yet do link-aware resolution before prefix matching.
