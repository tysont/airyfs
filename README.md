...
In-container visibility of mounted subtrees (so `exec` sees another volume's data through FUSE) is designed as one nested `agentfs` FUSE daemon per volume grafted over the mountpoint's stub directory, with the guest volume's Hrana traffic proxied through the host Durable Object to the target's SQLite so it never leaves the deployment. That guest data path is implemented and unit-tested but currently gated off (`GUEST_FUSE_ENABLED`): allocating a bridge port pair per mount destabilizes the Container instance on the present platform, so it needs the guest channels multiplexed over the existing bridge connection rather than new per-mount ports. Until then, a mounted subtree appears inside the Container as an empty stub directory — read and write mounted data through the direct-path plane, which is fully live.
